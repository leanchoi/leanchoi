# Esquel Rutas — Visor de rutas turísticas georreferenciadas

Aplicación web **self-hosted y dockerizada** para publicar y navegar rutas
turísticas georreferenciadas con contenido multimedia: **track GPX + puntos de
interés (POIs)** con descripción, foto y audioguía + **perfil de altimetría**
sincronizado con el mapa. Inspirada en la mecánica de *toprutas.com*, corregidos
sus errores (nada hardcodeado, POIs cargados on-demand por API, tiles gratuitos
con atribución, un solo puerto expuesto).
...
- **Stack:** TypeScript `strict` en todo. API con Fastify 4 + Drizzle ORM +
  ...
  TypeScript vanilla + Leaflet 1.9 + `@raruto/leaflet-elevation` (altimetría D3) +
  Swiper (galerías) + `marked`/`dompurify` (markdown saneado).
...
/db/seed             seed.ts (ruta demo de Esquel con GPX sintético)
...
- **`routes`** — metadata + stats calculadas del GPX (`distance_m`, `ascent_m`,
  `descent_m`, bbox, center) + `status` (`draft`/`published`) + `slug`.
...
```bash
git clone <repo> esquel-rutas && cd esquel-rutas
cp .env.example .env
...
docker compose exec api npm run seed  # (opcional) carga la ruta demo de Esquel
