...
export default function TableApp({
  ...
  const readOnly = (data as any)?.readOnly as string | null
  const canEdit = data ? ['owner', 'editor'].includes(data.myRole) && !readOnly : false
...
function ShareModal({
  ...
  const load = useCallback(async () => {
    ...
    if (ur.ok) {
      // /api/users de Trochi devuelve un array con display_name
      const list = await ur.json()
      setUsers(
        (Array.isArray(list) ? list : list.users || []).map((u: any) => ({
          ...u,
      ...
      <div className="space-y-2">
        {ownerUser && (
          <div className="flex items-center gap-2 rounded-lg border border-slate-800 px-3 py-2">
            <Avatar name={ownerUser.name} avatar={ownerUser.avatar} userId={ownerUser.id} size={28} />
        ...
        {collabs.map((c) => (
          <div key={c.id} className="flex items-center gap-2 rounded-lg border border-slate-800 px-3 py-2">
            <Avatar name={c.name} avatar={c.avatar} userId={c.user_id} size={28} />
