...
export async function GET(_req: Request, { params }: { params: { recordId: string } }) {
  ...
  const comments = db
    .prepare(
      `SELECT c.*, u.display_name AS user_name, u.avatar AS user_avatar, u.id AS user_num_id
       FROM record_comments c JOIN users u ON u.id = c.user_id
...
export async function POST(req: Request, { params }: { params: { recordId: string } }) {
  ...
  const id = uid();
  db.prepare(
    'INSERT INTO record_comments (id, record_id, table_id, user_id, body) VALUES (?, ?, ?, ?, ?)'
  ).run(id, params.recordId, loc.tableId, user.id, text);

  const table = db.prepare('SELECT name FROM base_tables WHERE id = ?').get(loc.tableId) as any;
  notifyMentions({
    text,
    actorId: user.id,
    actorName: user.name,
  ...
  const rec = db
    .prepare('SELECT created_by FROM base_records WHERE id = ?')
    .get(params.recordId) as any;
  if (rec?.created_by) {
    notify({
