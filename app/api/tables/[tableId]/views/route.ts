...
export async function POST(req: Request, { params }: { params: { tableId: string } }) {
  ...
  const max = db
    .prepare('SELECT COALESCE(MAX(position), -1) AS m FROM base_views WHERE table_id = ?')
    .get(params.tableId) as any;
  const id = uid();
  db.prepare(
    'INSERT INTO base_views (id, table_id, name, type, config, position, created_by, personal) VALUES (?, ?, ?, ?, ?, ?, ?, ?)'
