...
function viewBase(viewId: string) {
  const v = db.prepare('SELECT * FROM base_views WHERE id = ?').get(viewId) as any;
  if (!v) return null;
...
export async function PATCH(req: Request, { params }: { params: { viewId: string } }) {
  ...
  const role = baseRole(user, vb.baseId);
  ...
  const config = body.config !== undefined ? JSON.stringify(body.config) : vb.view.config;
  db.prepare('UPDATE base_views SET name = ?, config = ? WHERE id = ?').run(
    name,
    config,
    params.viewId
  );
...
export async function DELETE(_req: Request, { params }: { params: { viewId: string } }) {
  ...
  const count = db
    .prepare('SELECT COUNT(*) AS c FROM base_views WHERE table_id = ? AND personal = 0')
    .get(vb.view.table_id) as any;
  if (!vb.view.personal && count.c <= 1) return err('No podés borrar la única vista compartida');
  db.prepare('DELETE FROM base_views WHERE id = ?').run(params.viewId);
  return json({ ok: true });
}
