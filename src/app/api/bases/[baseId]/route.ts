...
export async function GET(_req: Request, { params }: { params: { baseId: string } }) {
  ...
  const tables = (
    db.prepare('SELECT * FROM tables WHERE base_id = ? ORDER BY position').all(params.baseId) as any[]
  ).map((t) => ({
    ...t,
    views: (
      db.prepare('SELECT * FROM views WHERE table_id = ? ORDER BY position').all(t.id) as any[]
    )
  ...
  const collaborators = db
    .prepare(
      `SELECT c.id, c.user_id, c.role, u.name, u.username, u.avatar
       FROM collaborators c JOIN users u ON u.id = c.user_id WHERE c.base_id = ?`
    )
...
export async function DELETE(_req: Request, { params }: { params: { baseId: string } }) {
  ...
  const role = baseRole(user.id, params.baseId, user.role)
  if (!isOwner(role)) return forbidden()
  const tables = db.prepare('SELECT id FROM tables WHERE base_id = ?').all(params.baseId) as any[]
  const tx = db.transaction(() => {
    for (const t of tables) {
      db.prepare('DELETE FROM fields WHERE table_id = ?').run(t.id)
      db.prepare('DELETE FROM views WHERE table_id = ?').run(t.id)
      db.prepare('DELETE FROM comments WHERE table_id = ?').run(t.id)
      db.prepare('DELETE FROM records WHERE table_id = ?').run(t.id)
    }
    db.prepare('DELETE FROM tables WHERE base_id = ?').run(params.baseId)
    db.prepare('DELETE FROM collaborators WHERE base_id = ?').run(params.baseId)
