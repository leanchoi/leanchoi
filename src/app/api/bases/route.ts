...
export async function GET() {
  ...
  const bases = accessibleBases(user).map((b) => {
    const tables = db
      ...
      .all(b.id) as any[]
    const owner = db.prepare('SELECT name FROM users WHERE id = ?').get(b.owner_id) as any
    return { ...b, tables, ownerName: owner?.name || '¿?', mine: b.owner_id === user.id }
  })
  return json({ bases })
}
...
export async function POST(req: Request) {
  ...
  const tx = db.transaction(() => {
    ...
    db.prepare(
      `INSERT INTO fields (id, table_id, name, type, options, position) VALUES (?, ?, 'Notas', 'longtext', '{}', 1)`
    ).run(uid(), tableId)
    db.prepare(
